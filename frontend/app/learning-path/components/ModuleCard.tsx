...
export default function ModuleCard({
  ...
  return (
    <div
      className={`relative flex w-full max-w-4xl mx-auto my-12 ${alignment === "left" ? "justify-start" : "justify-end"}`}
    >
      ...
      >
        <div
          className={`w-12 h-12 rounded-full border-4 flex items-center justify-center z-10 shadow-lg 
            ${
              isCompleted
      ...
      >
        ...
        <div
          ...
          ${
            isCompleted
              ? "from-[#22C55E]/50 to-[#22C55E]/10"
              : isInProgress
                ? "from-[#F58320]/50 to-[#F58320]/10"
        ...
        <div className="p-6 pb-4 border-b border-[#2A2B4A]/50">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h3
              className={`text-lg font-bold ${isLocked ? "text-[#8E90B0]" : "text-white"}`}
            >
              {title}
            </h3>
          </div>
        ...
        <div className="p-6 pt-4 space-y-4">
          ...
          {isInProgress && (
            <div className="mt-6">
              <div className="flex justify-between text-xs mb-2">
                ...
                <span className="text-[#F58320] font-bold">
                  {progressPercentage}%
                </span>
              </div>
          ...
          <div className="mt-6 pt-2">
            {isLocked ? (
              ...
              <Link
                href={`/ai-tutor?topic=${encodeURIComponent(title)}&courseId=${id}${nextCourseTitle ? `&nextCourse=${encodeURIComponent(nextCourseTitle)}` : ""}`}
              ...
              >
                <button
                  className={`w-full py-3 rounded-lg font-semibold flex items-center justify-center gap-2 transition-all
                    ${
                      isCompleted
                        ? "bg-[#2A2B4A] text-white hover:bg-[#2A2B4A]/80"
                        : isInProgress
                          ? "bg-[#F58320] text-white hover:bg-[#F58320]/90 shadow-[0_0_15px_rgba(245,131,32,0.3)]"
                          : ""
                    }`}
                >
                  {isCompleted ? (
